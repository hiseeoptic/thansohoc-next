...
const Chatbot: React.FC<ChatbotProps> = ({ sharedResults, sheetData, onClose, language }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      // Xây dựng context từ sharedResults — bao gồm cả kiến thức sâu về từng số
      ...
      const deepContext = buildDeepContext();
      ...
      const context = sharedResults ? `
        ...
        **KIẾN THỨC SÂU VỀ CÁC SỐ CỦA NGƯỜI DÙNG (BẮT BUỘC SỬ DỤNG KHI PHÂN TÍCH):**
        ${deepContext}
