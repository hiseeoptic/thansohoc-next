...
// === THÊM ĐOẠN NÀY VÀO ĐÂY (sau import, trước interface) ===
const getTranslations = (lang: 'vi' | 'en') => ({
  title: lang === 'vi' ? "Phân Tích Ma Trận Chuyên Sâu" : "Advanced Matrix Analysis",
  subtitle: lang === 'vi' ? "AI Engine v4.0: Logic Quy Tắc + Tâm lý Hành vi" : "AI Engine v4.0: Rule-Based Logic + Behavioral Psychology",
  guidanceTitle: lang === 'vi' ? "Hướng dẫn chọn chỉ số để kết nối" : "Guide to Choosing Numbers for Connection",
  guidance1: lang === 'vi'
    ? "Kết hợp Đường Đời + Nội Tâm + Sứ Mệnh (hoặc bất kỳ 2 trong 3) để biết xu hướng cuộc đời, mô hình thành công, thử thách lớn..."
    : "Combine Life Path + Heart Desire + Mission (or any 2 of them) to understand life trends, success patterns, major challenges...",
  guidance2: lang === 'vi'
    ? "Kết hợp Nội Tâm + Thái Độ + Nhân Cách + Trưởng Thành để hiểu tính cách cốt lõi, phản ứng bản năng, cách tương tác xã hội..."
    : "Combine Heart Desire + Attitude + Personality + Maturity to understand core personality, instinctive reactions, social interaction style...",
  subscriptionLabel: lang === 'vi' ? "Nhập Mã Thuê Bao:" : "Enter Subscription Code:",
  subscriptionPlaceholder: lang === 'vi' ? "Nhập mã thuê bao của bạn (ví dụ: 123123)" : "Enter your subscription code (e.g. 123123)",
  mode2: lang === 'vi' ? "2 Chỉ Số" : "2 Numbers",
  mode3: lang === 'vi' ? "3 Chỉ Số" : "3 Numbers",
  analyzeBtn: lang === 'vi' ? "Phân Tích Chuyên Sâu" : "Deep Analysis",
  analyzing: lang === 'vi' ? "Đang phân tích..." : "Analyzing...",
  askAI: lang === 'vi' ? "Hỏi AI về kết quả này" : "Ask AI about this result"
});
...
const ConnectionTool: React.FC<ConnectionToolProps> = ({ sheetData: initialSheetData, sharedResults, language }) => {
  ...
  const basicAnalysis = analyzeConnectionLogic(activeInputs, language);
  ...
  try {
      ...
      const inputDescriptions = activeInputs.map(i => `${i.type} (${i.value})`).join(" + ");
       const langInstruction = language === 'en'
        ? "Respond ENTIRELY in natural, professional English using behavioral psychology terms. Use real-life examples from work, family, finance, personal growth. Do NOT use Vietnamese."
        : "Đóng vai: Bạn là Chuyên gia Tâm lý học Hành vi và Cố vấn Chiến lược Nhân sự. Phân tích sâu, chi tiết, dùng tiếng Việt, bám sát rule engine và dữ liệu cung cấp, sử dụng ví dụ thực tế từ công việc, gia đình, tài chính, phát triển bản thân.";
      let prompt = "";
