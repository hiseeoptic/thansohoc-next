...
<ul>
    ...
    // Gọi OpenAI trực tiếp server-side (thay fetch /api/analyze)
const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY,
});

try {
  // ... giữ nguyên build prompt cũ của anh (prompt dài 5 lớp/8 tầng) ...
  ...
  const responseText = completion.choices[0]?.message?.content || '<p class="text-yellow-400">Không nhận được nội dung phân tích từ AI. Vui lòng thử lại.</p>';
